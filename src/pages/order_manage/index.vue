<template>
  <div class="index">
    ...
    <div
      ...
      :key="index"
      @longpress="item.status <= 3 && longCheckItem(item.order_id, item.status,item.seller_id,item.buyer_id)"
    >
  ...
  methods: {
    ...
    longCheckItem (orderID, status, sellerID, buyerID) {
      if (status === 0) {
        ...
        // 待发货状态 （0）--> 待收货状态（1）
        wx.showActionSheet({
          itemList: ['确认是否退货', '拒绝退货并返回至[待发货状态]',
            '拒绝退货并返回至[已发货状态]', '拒绝退货并返回至[已收货状态]'],
          success: (res) => {
            if (res.tapIndex === 0) {
    ...
    // 确认发货状态
    async deliverGoods (orderID) {
      const { data } = await this.$fly.post('/order/deliverGoods', {
        order_id: orderID
      })
