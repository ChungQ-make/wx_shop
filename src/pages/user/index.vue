<template>
  <div class="indexContainer">
      <button open-type="getUserInfo" @getuserinfo="login" :hidden='isShow'>登陆</button>
    ...
    <view class="user_main">
        ...
        <!-- 收货地址管理 -->
        <view class="address_wrap" @click="handleChooseAdress">收货地址管理</view>
        <!-- 应用信息相关 -->
        ...
        <view class="recommend_wrap sw2" v-show='isShow' @click="logout">退出</view>
    </view>
</view>
  </div>
</template>
        ...
        wx.login({
          success: async (res1) => {
            ...
            wx.getUserInfo({
              success: async (res2) => {
                const {userInfo} = res2
                const data = await this.$fly.post('/user/login', {code, userInfo})
...
page,view,text,swiper,swper-item,imgage,navigator{
    padding: 0;
    margin: 0;
    /* 重要 */
